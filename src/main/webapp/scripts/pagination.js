$(document).ready(function () {
    ...
    var pagination_items = $('.douban-collections').find('.douban-collection');
    var total_items = pagination_items.size();
    ...
    while (total_pages > current_link) {
        navigation_html += '<a class="page_link" href="javascript:go_to_page(' + current_link + ')" ' +
                           'data-page-number="' + current_link + '">' + (current_link + 1) + '</a>';
        current_link++;
    }
    ...
    pagination_items.slice(0, items_per_page).show();
});
...
function go_to_page(page_number) {
    ...
    var current_active_page = $('.page_link[data-page-number=' + page_number + ']');
    current_active_page.addClass('active_page');
    current_active_page.siblings('.active_page').removeClass('active_page');
